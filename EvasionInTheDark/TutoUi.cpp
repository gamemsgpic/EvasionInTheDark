...
void TutoUi::Update(float dt)
{
	...
	if (tutoIndex == 0)
	{
		...
		{
			SoundMgr::Instance().CanStopPlaySfx(SOUNDBUFFER_MGR.Get("sound/tutopage0.wav"))->setVolume(10.f);
			explainStart = 0.f;
		}
		explainStart = 0.f;

		body.setTexture(TEXTURE_MGR.Get("graphics/Tuto_1.png"));
		text1.setString(L"        안녕하세요. 지금 계신곳은 튜토리얼 화면입니다.\n 방향키 왼쪽과 오른쪽으로 페이지를 전환 할 수 있습니다.\n           엔터를 누르시면 타이틀 화면으로 돌아갑니다.");
	...
	if (tutoIndex == 1)
	{
		...
		body.setTexture(TEXTURE_MGR.Get("graphics/Tuto_1.png"));
		text1.setString(L"캐릭터 조작키는 방향키 왼쪽 그리고 오른쪽 입니다.");
		text1.setOrigin(text1.getGlobalBounds().width * 0.5f, text1.getGlobalBounds().height * 0.5f);
	}
	if (tutoIndex == 2)
	{
	...
	if (tutoIndex == 5)
	{
		...
		text1.setOrigin(text1.getGlobalBounds().width * 0.5f, text1.getGlobalBounds().height * 0.5f);
		if (soundStartTrigger > soundStartDelay - 2.f)
		{
	...
	if (tutoIndex == 17)
	{
		...
		text1.setString(L"게임오버가 되었을 때 Enter를 누르면 타이틀 화면으로 돌아갑니다.");
		text1.setOrigin(text1.getGlobalBounds().width * 0.5f, text1.getGlobalBounds().height * 0.5f);
	}
	if (tutoIndex == 18)
	{
